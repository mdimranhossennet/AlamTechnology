...
async function seed() {
  try {
    ...
    const imageDir = path.join(__dirname, '../public/assets/images/products');
    let imageFiles = [];
    if (fs.existsSync(imageDir)) {
      imageFiles = fs.readdirSync(imageDir).filter(f => f.match(/\.(webp|jpg|jpeg|png|gif)$/i));
    }
    if (imageFiles.length === 0) {
      console.warn("No images found in public/assets/images/products.");
      imageFiles = ['placeholder.webp'];
    }
    ...
    const finalProducts = baseProducts.map((p, idx) => {
      ...
      // Use exactly the images from the folder
      const img1 = imageFiles[idx % imageFiles.length];
      const img2 = imageFiles[(idx + 5) % imageFiles.length];
      
      productCopy.images = [
        `/assets/images/products/${img1}`,
        `/assets/images/products/${img2}`
      ];
